...
class TauLeaping():
    ...
    def sample(self, model, N, num_intermediates, x = None):
        t = 1.0
        C,H,W = self.cfg.data.shape
        D = C*H*W
        S = self.cfg.data.S
        ...
        DPF_type = scfg.DPF_type if hasattr(scfg, 'DPF_type') else 0
        print('DPF_type: ', DPF_type)
        ...
        with torch.no_grad():
            ...
            for idx, t in tqdm(enumerate(ts[0:-1])):
                ...
                if DPF_type==0:
                    ...
                    reverse_rates = forward_rates * (inner_sum - 1) # (N, D, S)
...
class ConditionalTauLeaping():
    ...
    def sample(self, model, N, num_intermediates, conditioner, x = None):
        ...
        with torch.no_grad():
            if x == None:
                x = get_initial_samples(N, sample_D, device, S, initial_dist, initial_dist_std)
