...
out = np.zeros((32*num_samples, 32 * num_variance, 3), dtype=np.uint8)

x = torch.from_numpy(np.load('./visualization_xT.npy')).to('cuda')
for i in range(0, 20):
    ...
    for j in range(num_variance):
        out[i * 32:(i + 1) * 32, j * 32:(j + 1) * 32] = res[j]

cv2.imwrite('./visualization_'+str(eval_cfg.DPF_type)+'.png', out[:,:,::-1])
